...
$status=$_POST["status"];
$id=$_POST["id"];
$CompanyData="select CompanyName,compliance_status from tw_company_details where id='".$id."'";
...
$compliance_status = $decodedJSON->response[1]->compliance_status;
...
$StatusValue= $sign->SelectF($GetStatus,"verification_status");
	...
	if($status==$compliance_status){
		echo "Exist";
	}
	else if($retVal1=="Success"){
		$mailobj=new twMail();
		$subject = "Verification Result";
		$myfile = fopen($settingValueMailPath."pgUpdateComplianceStatus.html", "r");
		$message = fread($myfile,filesize($settingValueMailPath."pgUpdateComplianceStatus.html"));
