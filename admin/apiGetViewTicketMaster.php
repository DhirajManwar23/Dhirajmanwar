...
if (empty($status)){
	$qry="select ts.id, ts.subject, ts.ticket_date,tvm.verification_status from tw_ticket_system ts INNER JOIN tw_verification_status_master tvm ON ts.status = tvm.id order by id Desc";
	$qry1="select count(*) as cnt from tw_ticket_system";
}else {
	echo $qry="select ts.id, ts.subject, ts.ticket_date,tvm.verification_status from tw_ticket_system ts INNER JOIN tw_verification_status_master tvm ON ts.status = tvm.id where status = '".$status."' order by id Desc";
