...
if($requesttype=="add"){
	...
	$reasonQry="SELECT id,reason FROM tw_rejected_reason_master where visibility='true' AND panel='".$settingValueComplianceDoc."' order by priority asc";
			...
			<div class="card-body">
			  ...
			  <div class="forms-sample">
				...
				<div class="form-group row">
				  ...
				  <textarea  class="col-sm-9" id="txtDocument_Number" <?php if($requesttype=="edit"){ echo "disabled";} ?> maxlength="200" rows="4"  placeholder="Description"><?php echo $document_number; ?></textarea>
				</div>
				<div class="form-group row">
				<?php if($requesttype=="edit"){ ?>
...
function RejectionText(reqid)
{
	...
	$.ajax({
		type:"POST",
		url:"apiComplianceRejectDocument.php",
		data:{Msg:msg,id:reqid},
		success:function(response){	
			console.log(response);
			if($.trim(response)=="Success"){
				
					showAlertRedirect("Success","Document Rejected Successfully","success","pgCompanyProfile.php?id="+valcreated_by);
...
function adddata(){
		...
		setErrorOnBlur("Document_Proof");
				...
				$("#ERROR").fadeIn();
				$("#EXIST").html("Documents already uploded");
				$("#ERROR").fadeOut(5000);
			}
			...
			var valquery = "insert into  tw_compliance_document(company_id,document_type,document_number,document_proof,document_verification_status,document_size,document_purpose,created_by,created_on,created_ip)values('"+valcreated_by+"','"+$("#selDocument_Type").val()+"','"+$("#txtDocument_Number").val()+"','"+hdnIDimg+"','"+settingValueDocumentPendingStatus+"','"+hdnIDfsize+"','"+valdocumentpurpose+"','"+valcreated_by+"','"+valcreated_on+"','"+valcreated_ip+"')";
			
			var valquerycount = "select count(*) as cnt from tw_compliance_document where document_type = '"+$("#selDocument_Type").val()+"' and company_id='"+valcreated_by+"'";
		     }
		else{
...
function accept(reqid){
	  ...
	  var msg=$("#ReasonMsg").val();
		...
		if(valrequesttype=="add"){
			...
			var valquerycount = "select count(*) as cnt from tw_employee_document where document_type = '"+$("#selDocument_Type").val()+"' and employee_id='"+valcreated_by+"'";
		...
		$.ajax({
		type:"POST",
		url:"apiComplianceAcceptDocument.php",
