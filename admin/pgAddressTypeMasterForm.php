...
$visibility = "";
if($requesttype=="edit"){
	$qry = "SELECT address_type_value,address_icon,priority,description,visibility from tw_address_type_master WHERE id = ".$id." ";
	$retVal = $sign->FunctionJSON($qry);
	$decodedJSON = json_decode($retVal);
	$address_type_value = $decodedJSON->response[0]->address_type_value;
	$address_icon = $decodedJSON->response[1]->address_icon;
...
<title>Trace Waste | Address Type Master</title>
<!-- plugins:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Address Type Master</h4>
					  <div class="forms-sample">
						<div class="form-group">
							<label for="txtAddType">Address Type <code>*</code></label>
							<input type="text" class="form-control" id="txtAddType" maxlength="20" value="<?php echo $address_type_value; ?>" placeholder="Address Type" />
						</div>
						<div class="form-group">
							<label for="txtAddIcon">Address Icon <code>*</code></label>
							<input type="text" class="form-control" id="txtAddIcon" maxlength="20" value="<?php echo $address_icon; ?>" placeholder="Address Icon" />
						</div>
						<div class="form-group">
...
function addrecord(){
		var buttonHtml = $('#btnAddrecord').html();
		 if(!validateBlank($("#txtAddType").val())){
			setErrorOnBlur("txtAddType");
		  }
		  else if(!validateBlank($("#txtAddIcon").val())){
			setErrorOnBlur("txtAddIcon");
		  } 
		...
		$.ajax({
			type:"POST",
			url:"apiAddAddressTypeMaster.php",
			data:{address_type_value:$("#txtAddType").val(),address_icon:$("#txtAddIcon").val(),priority:$("#txtPriority").val(),description:$("#txtDescription").val(),visibility:$('#chkVisibility').prop('checked')},
			success:function(response){
				...
				if($.trim(response)=="Success"){
					if(valrequesttype=="add"){
						...
						showAlertRedirect("Success","Record Added Successfully","success","pgAddressTypeMaster.php");
						}
					else{
						$('#btnAddrecord').html('Update Record');
						showAlertRedirect("Success","Record Updated Successfully","success","pgAddressTypeMaster.php");
						}
