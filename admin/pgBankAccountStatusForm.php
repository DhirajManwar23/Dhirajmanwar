...
$bank_account_status_value = "";
$priority = ""; 
$description = "";
$visibility = "";
if($requesttype=="edit"){
	$qry = "SELECT bank_account_status_value,priority,description,visibility from  tw_bank_account_status_master WHERE id = ".$id." ";
	$retVal = $sign->FunctionJSON($qry);
	$decodedJSON = json_decode($retVal);
	$bank_account_status_value = $decodedJSON->response[0]->bank_account_status_value;
...
<title>Trace Waste | Bank Account Status Master</title>
<!-- plugins:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Bank Account Status Master</h4>
                  <div class="forms-sample">
                    <div class="form-group">
                      <label for="txtbankstatus">Bank Account Status <code>*</code></label>
                      <input type="text" class="form-control" id="txtbankstatus" maxlength="20" value="<?php echo $bank_account_status_value; ?>" placeholder="Bank Status" />
                    </div>
                    <div class="form-group">
...
function addrecord(){
		...
		if(!validateBlank($("#txtbankstatus").val())){
			setErrorOnBlur("txtbankstatus");
		  }
			...
			$.ajax({
				type:"POST",
				url:"apiAddBankAccountStatusMaster.php",
				data:{bank_account_status_value:$("#txtbankstatus").val(),priority:$("#txtPriority").val(),description:$("#txtDescription").val(),visibility:$('#chkVisibility').prop('checked')},
				...
				console.log(response);
					...
					if($.trim(response)=="Success"){
						if(valrequesttype=="add"){
							...
							showAlertRedirect("Success","Record Added Successfully","success","pgBankAccountStatusMaster.php");
							}
						else{
							$('#btnAddrecord').html('Update Record');
							showAlertRedirect("Success","Record Updated Successfully","success","pgBankAccountStatusMaster.php");
						}
						$("#txtbankstatus").val("");
						$("#txtPriority").val("");
						$("#txtDescription").val("");
