...
if($requesttype=="edit"){
	...
	$qry = "SELECT 	bank_account_type_value,priority,description,visibility from  tw_bank_account_type_master WHERE id = ".$id."";
	$retVal = $sign->FunctionJSON($qry);
	$decodedJSON = json_decode($retVal);
	$bank_account_type_value = $decodedJSON->response[0]->bank_account_type_value;
                  ...
                  <h4 class="card-title">Bank Account Type Master</h4>
					 <div class="forms-sample">
						<div class="form-group">
						  <label for="txtBankAccountType">Bank Account Type <code>*</code></label>
						  <input type="text" class="form-control" id="txtBankAccountType" maxlength="20" value="<?php echo $bank_account_type_value; ?>" placeholder="Bank Account Type" />
						</div>
						<div class="form-group">
...
function addrecord(){
		var buttonHtml = $('#btnAddrecord').html();
		 if(!validateBlank($("#txtBankAccountType").val())){
			setErrorOnBlur("txtBankAccountType");
		  }
			...
			$.ajax({
				type:"POST",
				url:"apiAddBankAccountTypeMasterStatus.php",
				data:{bankaccounttype:$("#txtBankAccountType").val(),priority:$("#txtPriority").val(),description:$("#txtDescription").val(),visibility:$('#chkVisibility').prop('checked')},
				...
				console.log(response);
					...
					if($.trim(response)=="Success"){
						if(valrequesttype=="add"){
							...
							showAlertRedirect("Success","Record Added Successfully","success","pgBankAccountTypeMaster.php");
							}
						else{
							$('#btnAddrecord').html('Update Record');
							showAlertRedirect("Success","Record Updated Successfully","success","pgBankAccountTypeMaster.php");	
						}
						$("#txtBankAccountType").val("");
						$("#txtPriority").val("");
						$("#txtDescription").val("");
					}
