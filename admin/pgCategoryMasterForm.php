...
if($requesttype=="edit"){
	...
	$qry = "SELECT category_name,priority,description,visibility from tw_category_master WHERE id = ".$id." ";
                  ...
                  <h4 class="card-title">Category Master</h4>
					<div class="forms-sample">
						<div class="form-group">
						  ...
						  <input type="text" class="form-control" id="txtCategoryName" maxlength="20" value="<?php echo $category_name; ?>" placeholder="Category Name" />
						</div>
						<div class="form-group">
...
function addrecord(){
	var buttonHtml = $('#btnAddrecord').html();
	 if(!validateBlank($("#txtCategoryName").val())){
		...
		$.ajax({
			type:"POST",
			url:"apiAddCategoryMasterStatus.php",
			data:{categoryname:$("#txtCategoryName").val(),priority:$("#txtPriority").val(),description:$("#txtDescription").val(),visibility:$('#chkVisibility').prop('checked')},
			success:function(response){
				...
				if($.trim(response)=="Success"){
					if(valrequesttype=="add"){
						...
						showAlertRedirect("Success","Record Added Successfully","success","pgCategoryMaster.php");
						
						}
					else{
						$('#btnAddrecord').html('Update Record');
						showAlertRedirect("Success","Record Updated Successfully","success","pgCategoryMaster.php");
					}
					$("#txtCategoryName").val("");
					$("#txtPriority").val("");
					$("#txtDescription").val("");
