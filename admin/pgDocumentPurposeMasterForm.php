...
if($requesttype=="edit"){
	...
	$qry = "SELECT document_purpose_value,priority,description,visibility from tw_document_purpose_master WHERE id = ".$id." ";
	$retVal = $sign->FunctionJSON($qry);
	$decodedJSON = json_decode($retVal);
	$document_purpose_value	 = $decodedJSON->response[0]->document_purpose_value	;
...
<title>Trace Waste | Document Purpose Master</title>
<!-- plugins:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Document Purpose Master</h4>
					  <div class="forms-sample">
						<div class="form-group">
							<label for="txtDocumentPurpose">Document Purpose Value <code>*</code></label>
							<input type="text" class="form-control" id="txtdocument_purpose_value" maxlength="20" value="<?php echo $document_purpose_value; ?>" placeholder="Document Purpose Value" />
						</div>
						<div class="form-group">
...
function addrecord(){
		...
		if(!validateBlank($("#txtdocument_purpose_value").val())){
			setErrorOnBlur("txtdocument_purpose_value");
		  } 
			...
			$.ajax({
				type:"POST",
				url:"apiAddDocumentPurposeMaster.php",
				data:{document_purpose_value:$("#txtdocument_purpose_value").val(),priority:$("#txtPriority").val(),description:$("#txtDescription").val(),visibility:$('#chkVisibility').prop('checked')},
				success:function(response){
					...
					if($.trim(response)=="Success"){
						if(valrequesttype=="add"){
							...
							showAlertRedirect("Success","Record Added Successfully","success","pgDocumentPurposeMaster.php");
							}
						else{
							$('#btnAddrecord').html('Update Record');
							showAlertRedirect("Success","Record Updated Successfully","success","pgDocumentPurposeMaster.php");
						}
						$("#txtdocument_purpose_value").val("");
						$("#txtPriority").val("");
						$("#txtDescription").val("");
					}
