...
if($requesttype=="edit"){
	...
	$qry = "SELECT 	document_type_value,priority,description,visibility from  tw_document_type_master WHERE id = ".$id." ";
	$retVal = $sign->FunctionJSON($qry);
	$decodedJSON = json_decode($retVal);
	$document_type_value = $decodedJSON->response[0]->document_type_value;
...
<title>Trace Waste | Document Type Master</title>
<!-- plugins:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Document Type Master</h4>
					<div class="forms-sample">
						<div class="form-group">
							<label for="txtDocumentType">Document Type <code>*</code></label>
							<input type="text" class="form-control" id="txtDocumentType" maxlength="50" value="<?php echo $document_type_value; ?>" placeholder="Document Type" />
						</div>
						<div class="form-group">
...
function addrecord(){
		var buttonHtml = $('#btnAddrecord').html();
		 if(!validateBlank($("#txtDocumentType").val())){
			...
			$.ajax({
				type:"POST",
				url:"apiAddDocumentTypeMasterStatus.php",
				data:{documenttype:$("#txtDocumentType").val(),priority:$("#txtPriority").val(),description:$("#txtDescription").val(),visibility:$('#chkVisibility').prop('checked')},
				...
				console.log(response);
					...
					if($.trim(response)=="Success"){
						if(valrequesttype=="add"){
							...
							showAlertRedirect("Success","Record Added Successfully","success","pgDocumentTypeMaster.php");
							}
						else{
							$('#btnAddrecord').html('Update Record');
							showAlertRedirect("Success","Record Updated Successfully","success","pgDocumentTypeMaster.php");
						}
						$("#txtDocumentType").val("");
						$("#txtPriority").val("");
						$("#txtDescription").val("");
