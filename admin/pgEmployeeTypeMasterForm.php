...
if($requesttype=="edit"){
	...
	$qry = "SELECT employee_type_value,priority,description,visibility from tw_employee_type_master WHERE id = ".$id."";
	$retVal = $sign->FunctionJSON($qry);
	$decodedJSON = json_decode($retVal);
	$employee_type_value = $decodedJSON->response[0]->employee_type_value;
...
<title>Trace Waste | Employee Type Master</title>
<!-- plugins:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Employee Type Master</h4>
					  <div class="forms-sample">
						<div class="form-group">
							<label for="txtEmployeeType">Employee Type <code>*</code></label>
							<input type="textEmployee" class="form-control" id="txtEmployeeType" maxlength="20"  value="<?php echo $employee_type_value; ?>"placeholder="Employee Type" />
						</div>
						<div class="form-group">
...
function addrecord(){
		...
		if(!validateBlank($("#txtEmployeeType").val())){
			setErrorOnBlur("txtEmployeeType");
		  }
			...
			$.ajax({
				type:"POST",
				url:"apiAddEmployeeTypeMasterStatus.php",
				data:{employee_type_value:$("#txtEmployeeType").val(),priority:$("#txtPriority").val(),description:$("#txtDescription").val(),visibility:$('#chkVisibility').prop('checked')},
				success:function(response){
					...
					if($.trim(response)=="Success"){
						if(valrequesttype=="add"){
							...
							showAlertRedirect("Success","Record Added Successfully","success","pgEmployeeTypeMaster.php");
						}
						else{
							$('#btnAddrecord').html('Update Record');
							showAlertRedirect("Success","Record Updated Successfully","success","pgEmployeeTypeMaster.php");
						}
						$("#txtEmployeeType").val("");
						$("#txtPriority").val("");
						$("#txtDescription").val("");
