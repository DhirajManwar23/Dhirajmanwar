...
if ($requesttype=="edit") {
$qry1 = "select id,panel,priority,description,visibility from tw_panel_master where id='".$requestid."' ORDER by priority,panel ASC";
$retVal = $sign->FunctionJSON($qry1);
...
<title>Trace Waste | Panel Master</title>
<!-- plugins:css -->
                ...
                <div class="card-body">
                  <h4 class="card-title">Panel master</h4>
					<div class="forms-sample">
						...
						<input type="text" class="form-control" id="txtpanel" maxlength="30" value="<?php echo $panel; ?>" placeholder="Panel" />
					</div>
					<br>
					...
					<div class="form-group">
					 ...
					 <div class="form-group">
						...
						<label class="switch">
							<input type="checkbox" id="chkvisibility" <?php if ($visibility=="true") 
								{ echo "checked"; } ?> />
...
function addrecord(){
	  var buttonHtml = $('#btnAddrecord').html();
		...
		if(valrequesttype=="add"){
			var valquery = "insert into tw_panel_master(panel,priority,description,visibility,created_by,created_on,created_ip)values('"+$("#txtpanel").val()+"','"+$("#txtPriority").val()+"','"+$("#txtDescription").val()+"','"+$('#chkvisibility').prop('checked')+"','"+valcreated_by+"','"+valcreated_on+"','"+valcreated_ip+"')";
			var valquerycount = "select count(*) as cnt from tw_panel_master where panel='"+$("#txtpanel").val()+"'";
		}
		else{
				var valrequestid = "" + "<?php echo $requestid;?>";
			
				var valquery = "Update tw_panel_master set panel='"+$("#txtpanel").val()+"',priority='"+$("#txtPriority").val()+"',description='"+$("#txtDescription").val()+"',visibility='"+$('#chkvisibility').prop('checked')+"',modified_by='"+valcreated_by+"',modified_on='"+valcreated_on+"',modified_ip='"+valcreated_ip+"' where id='"+valrequestid+"'";
		
				var valquerycount = "select count(*) as cnt from tw_panel_master where panel='"+$("#txtpanel").val()+"' and id!='"+valrequestid+"'";
		}
		$.ajax({
		type:"POST",
		...
		console.log(response);
			...
			if($.trim(response)=="Success"){
				if(valrequesttype=="add"){
					...
					showAlertRedirect("Success","Record Added Successfully","success","pgPanelMaster.php");
				}
				else{
					$('#btnAddrecord').html('Update Record');
					showAlertRedirect("Success","Record Updated Successfully","success","pgPanelMaster.php");
				}
			}
