...
if($requesttype=="edit")
{
	$qry="select name,priority,description,visibility,category_id,sub_category_id from tw_product_type_master where ID='".$requestid."' order by id Desc";
...
<title>Trace Waste | Material Type Master</title>
<!-- plugins:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Material Type Master</h4>
					...
					<div class="form-group">
						...
						<div class="form-group">
							...
							<select id="txtsubcategoryname" class="form-control" >
								<?php echo $retVal2;?>
							</select>
						</div>
						<div class="form-group">
...
$("#txtPriority").blur(function()
{
  ...
  function adddata(){
		...
		else if(!validateBlank($("#txtcategoryname").val())){
		...
		setErrorOnBlur("txtDescription");
			...
			if(valrequesttype=="add"){
				var valquery = "insert into  tw_product_type_master(category_id,sub_category_id,name,priority,description,visibility,created_by,created_on,created_ip) values('"+$("#txtcategoryname").val()+"','"+$("#txtsubcategoryname").val()+"','"+$("#txtname").val()+"','"+$("#txtPriority").val()+"','"+$("#txtDescription").val()+"','"+$('#chkVisibility').prop('checked')+"','"+valcreated_by+"','"+valcreated_on+"','"+valcreated_ip+"')";
				var valquerycount = "select count(*) as cnt from tw_product_type_master where name='"+$("#txtname").val()+"'";
			}
			else{
				var valrequestid = "<?php echo $requestid;?>";
				var valquery = "Update tw_product_type_master set category_id='"+$("#txtcategoryname").val()+"',sub_category_id='"+$("#txtsubcategoryname").val()+"', name='"+$("#txtname").val()+"',priority='"+$("#txtPriority").val()+"', description='"+$("#txtDescription").val()+"', visibility='"+$('#chkVisibility').prop('checked')+"', modified_by='"+valcreated_by+"', modified_on='"+valcreated_on+"', modified_ip='"+valcreated_ip+"' where id='"+valrequestid+"'";
				var valquerycount = "select count(*) as cnt from  tw_product_type_master where name='"+$("#txtname").val()+"' and ID!='"+valrequestid+"'";
			}
				...
				console.log(response);
					...
					if($.trim(response)=="Success"){
						if(valrequesttype=="add"){
							...
							showAlertRedirect("Success","Record Added Successfully","success","pgProductTypeMaster.php");
						}
						else{
							$('#btnAddrecord').html('Update Record');
							showAlertRedirect("Success","Record Updated Successfully","success","pgProductTypeMaster.php");
						}
					}
