...
if($requesttype=="edit"){
	...
	$qry = "SELECT tax_name,tax_value,priority,description,visibility from tw_tax_master WHERE id = ".$id." ";
	$retVal = $sign->FunctionJSON($qry);
	$decodedJSON = json_decode($retVal);
	$tax_name = $decodedJSON->response[0]->tax_name;
	$tax_value = $decodedJSON->response[1]->tax_value;
...
<title>Trace Waste | Tax Master</title>
<!-- plugins:css -->
                  ...
                  <h4 class="card-title">Tax Master</h4>
					<div class="forms-sample">
						<div class="form-group">
							<label for="txtTaxName">Tax Name <code>*</code></label>
							<input type="text" class="form-control" id="txtTaxName" maxlength="20" value="<?php echo $tax_name; ?>" placeholder="Tax Name" />
						</div>
						<div class="form-group">
							<label for="txtTaxValue">Tax Value <code>*</code></label>
							<input type="text" class="form-control" id="txtTaxValue" maxlength="3" value="<?php echo $tax_value; ?>" placeholder="Tax Value" />
						</div>
						<div class="form-group">
...
function addrecord(){
		var buttonHtml = $('#btnAddrecord').html();
		 if(!validateBlank($("#txtTaxName").val())){
			setErrorOnBlur("txtTaxName");
		  }
		  else if(!validateBlank($("#txtTaxValue").val())){
			setErrorOnBlur("txtTaxValue");
		  } 
			...
			$.ajax({
				type:"POST",
				url:"apiAddTaxMasterStatus.php",
				data:{tax_name:$("#txtTaxName").val(),tax_value:$("#txtTaxValue").val(),priority:$("#txtPriority").val(),description:$("#txtDescription").val(),visibility:$('#chkVisibility').prop('checked')},
				success:function(response){
					...
					if($.trim(response)=="Success"){
						if(valrequesttype=="add"){
							...
							showAlertRedirect("Success","Record Added Successfully","success","pgTaxMaster.php");
							}
						else{
							$('#btnAddrecord').html('Update Record');
							showAlertRedirect("Success","Record Updated Successfully","success","pgTaxMaster.php");
							}
