...
if($requesttype=="edit")
{
	$qry="select name,priority,description,visibility from tw_unit_of_measurement where ID='".$requestid."' order by id Desc";
...
<title>Trace Waste | Unit Of Measurement</title>
<!-- plugins:css -->
  ...
  function adddata(){
	    ...
	    if(!validateBlank($("#txtname").val())){
			...
			if(valrequesttype=="add"){
				var valquery = "insert into tw_unit_of_measurement(name,priority,description,visibility,created_by,created_on,created_ip) values('"+$("#txtname").val()+"','"+$("#txtPriority").val()+"','"+$("#txtDescription").val()+"','"+$('#chkVisibility').prop('checked')+"','"+valcreated_by+"','"+valcreated_on+"','"+valcreated_ip+"')";
				var valquerycount = "select count(*) as cnt from tw_unit_of_measurement where name='"+$("#txtname").val()+"'";
			}
			else{
				var valrequestid = "<?php echo $requestid;?>";
				var valquery = "Update tw_unit_of_measurement set name='"+$("#txtname").val()+"',priority='"+$("#txtPriority").val()+"', description='"+$("#txtDescription").val()+"', visibility='"+$('#chkVisibility').prop('checked')+"', modified_by='"+valcreated_by+"', modified_on='"+valcreated_on+"', modified_ip='"+valcreated_ip+"' where id='"+valrequestid+"'";
				var valquerycount = "select count(*) as cnt from tw_unit_of_measurement where name='"+$("#txtname").val()+"' and ID!='"+valrequestid+"'";
			}
			...
			success:function(response){
				...
				if($.trim(response)=="Success"){
					if(valrequesttype=="add"){
						...
						showAlertRedirect("Success","Record Added Successfully","success","pgUnitOfMeasurement.php");
					}
					else{
						$('#btnAddrecord').html('Update Record');
						showAlertRedirect("Success","Record Updated Successfully","success","pgUnitOfMeasurement.php");
					}
				}
