...
if($requesttype=="edit")
{
	$qry="select name,priority,description,visibility from tw_vehicle_type_master where ID='".$requestid."'";
...
<title>Trace Waste | Vehicles Type</title>
<!-- plugins:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Vehicles Type</h4>
					<div class="forms-sample">
						<div class="form-group">
							<label for="txtname">Name <code>*</code></label>
							<input type="text" class="form-control" id="txtname" maxlength="50" placeholder="Name" value="<?php echo $name; ?>">
						</div>
						<div class="form-group">
...
function adddata(){
	if(!validateBlank($("#txtname").val())){
		...
		if(valrequesttype=="add"){
			var valquery = "insert into tw_vehicle_type_master(name,priority,description,visibility,created_by,created_on,created_ip) values('"+$("#txtname").val()+"','"+$("#txtPriority").val()+"','"+$("#txtDescription").val()+"','"+$('#chkVisibility').prop('checked')+"','"+valcreated_by+"','"+valcreated_on+"','"+valcreated_ip+"')";
			var valquerycount = "select count(*) as cnt from tw_vehicle_type_master where name='"+$("#txtname").val()+"'";
		}
		else{
			var valrequestid = "<?php echo $requestid;?>";
			var valquery = "Update tw_vehicle_type_master set name='"+$("#txtname").val()+"',priority='"+$("#txtPriority").val()+"', description='"+$("#txtDescription").val()+"', visibility='"+$('#chkVisibility').prop('checked')+"', modified_by='"+valcreated_by+"', modified_on='"+valcreated_on+"', modified_ip='"+valcreated_ip+"' where id='"+valrequestid+"'";
			var valquerycount = "select count(*) as cnt from tw_vehicle_type_master where name='"+$("#txtname").val()+"' and ID!='"+valrequestid+"'";
		}
		...
		$.ajax({
			...
			success:function(response){
				...
				if($.trim(response)=="Success"){
					if(valrequesttype=="add"){
						showAlertRedirect("Success","Record Added Successfully","success","pgVehiclesTypeMaster.php");
					}
					else{
						showAlertRedirect("Success","Record Updated Successfully","success","pgVehiclesTypeMaster.php");
					}
				}
