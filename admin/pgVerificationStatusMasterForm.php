...
if($requesttype=="edit"){
	...
	$qry = "SELECT verification_status,priority,description,visibility from tw_verification_status_master WHERE id = ".$id." ";
	$retVal = $sign->FunctionJSON($qry);
	$decodedJSON = json_decode($retVal);
	$verification_status = $decodedJSON->response[0]->verification_status;
...
<title>Trace Waste | Verification Status Master</title>
<!-- plugins:css -->
        ...
        <div class="content-wrapper">
          <div class="row">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Verification Status Master</h4>             
					<div class="forms-sample">
						<div class="form-group">
						  <label for="txtVerificationStatus">Verification Status <code>*</code></label>
						  <input type="text" class="form-control" id="txtVerificationStatus" maxlength="20" value="<?php echo $verification_status; ?>" placeholder="Verification Status" />
						</div>
						<div class="form-group">
...
function addrecord(){
	var buttonHtml = $('#btnAddrecord').html();
	 if(!validateBlank($("#txtVerificationStatus").val())){
			setErrorOnBlur("txtVerificationStatus");
		  }
			...
			$.ajax({
				type:"POST",
				url:"apiAddVerificationStatusMaster.php",
				data:{verification_status:$("#txtVerificationStatus").val(),priority:$("#txtPriority").val(),description:$("#txtDescription").val(),visibility:$('#chkVisibility').prop('checked')},
				success:function(response){
					...
					if($.trim(response)=="Success"){
						if(valrequesttype=="add"){
							...
							showAlertRedirect("Success","Record Added Successfully","success","pgVerificationStatusMaster.php");
							}
						else{
							$('#btnAddrecord').html('Update Record');
							showAlertRedirect("Success","Record Updated Successfully","success","pgVerificationStatusMaster.php");
							}
						$("#txtVerificationStatus").val("");
						$("#txtPriority").val("");
						$("#txtDescription").val("");
					}
