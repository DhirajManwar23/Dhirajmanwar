...
$cur_date=date("Y-m-d h:i:sa");
 $qryMaterialID="Select id  from  tw_product_management  where id = '".$txtMaterialName."'";
$retValMaterialID = $sign->SelectF($qryMaterialID,"id");
...
$qry4="Select count(*) as cnt from tw_tax_invoice_temp  where employee_id = '".$employee_id."' and material_id='".$retValMaterialID."' and rate='".$txtRate."' ORDER BY id Asc";
...
if($retVal4!=1){
	...
	$qry="insert into tw_tax_invoice_temp  (employee_id,material_id,quantity,hsn,tax,rate,total,created_by,created_on,created_ip) values('".$employee_id."','".$retValMaterialID."','".$txtQuantity."','".$txtHSN."','".$txtTax."','".$txtRate."','".$txtTotalAmount."','".$employee_id."','".$cur_date."','".$ip_address."')";
	...
	$table.="</tr>";
		...
		$table.="<tr style='font-weight:bold'><td colspan='3'>Total</td>";
		...
		$exist = "exist";
		$responsearray=array();
		array_push($responsearray, $table,round($valfinaltotalamount,2), $valtotalquantity, $exist);
		echo json_encode($responsearray);
}
