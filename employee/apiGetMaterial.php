...
$productQry="SELECT product_id FROM `tw_po_details` where po_id='".$po_id."'";
$product=$sign->SelectF($productQry,"product_id");


if($productGet==0){
$MaterialQry="SELECT  moi.id as moi_id,mo.po_id,mo.company_id,mo.status,mo.company_address,moi.quantity,ca.state,ca.city,moi.material_id,pm.product_name,cd.CompanyName FROM tw_material_outward mo INNER JOIN tw_company_address ca ON mo.company_address=ca.id INNER JOIN tw_material_outward_individual moi ON mo.id=moi.material_outward_id INNER JOIN tw_product_management pm ON moi.material_id=pm.id INNER JOIN tw_company_details cd ON mo.company_id=cd.ID where (ca.state LIKE '%".$Fetchstate."%' or ca.city Like '%".$FetchCity."%') And mo.status='".$settingValueApprovedStatus."' AND moi.assign_status!='".$settingValuealloted."' ";


$qry2="SELECT count(*)as cnt  FROM tw_material_outward mo INNER JOIN tw_company_address ca ON mo.company_address=ca.id INNER JOIN tw_material_outward_individual moi ON mo.id=moi.material_outward_id INNER JOIN tw_product_management pm ON moi.material_id=pm.id INNER JOIN tw_company_details cd ON mo.company_id=cd.ID  where (ca.state LIKE '%".$Fetchstate."%' or ca.city Like '%".$FetchCity."%') And mo.status='".$settingValueApprovedStatus."' AND moi.assign_status!='".$settingValuealloted."'  ";
			...
			<div class='row'>
			    <div class='card-body p-3'>
						...
						<div class='content'>
								...
								<input class='cbCheck' type='checkbox' value='".$moi_id."/".$total_quantity."/".$company_id."/".$material_id."/".$Getstate."/".$Getcity."' id='check-".$moi_id."' /><i class='input-helper'></i>
