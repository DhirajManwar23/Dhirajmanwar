...
$qry="select po_id,fulfillment_cycle,description,state,district,supplier_part_number,product_id,quantity,start_date,	delivery_date,price_per_unit,amount from  tw_po_details WHERE po_id = '".$hdnOrderID."' ORDER BY id DESC";
$retVal = $sign->FunctionJSON($qry);

$qry1="Select count(*) as cnt from tw_po_details where po_id = '".$hdnOrderID."' ";
...
while($x>=$i){
	...
	$qry2="insert into tw_po_details_temp (company_id,fulfillment_cycle,description,state,district,supplier_part_number,product_id,quantity,start_date,delivery_date,price_per_unit,amount,created_by,created_ip,created_on) values('".$company_id."','".$fulfillment_cycle."','".$description."','".$state."','".$district."','".$supplier_part_number."','".$product_id."','".$quantity."','".$start_date."','".$delivery_date."','".$price_per_unit."','".$amount."','".$employee_id."','".$ip_address."','".$cur_date."' )";
...
$qry3="Select id,fulfillment_cycle,description,state,district,supplier_part_number,product_id,quantity,start_date,delivery_date,price_per_unit,amount from tw_po_details_temp where company_id = '".$company_id."' ORDER BY id DESC";
$retVal3 = $sign->FunctionJSON($qry3);


$qry4="Select count(*) as cnt from tw_po_details_temp where company_id = '".$company_id."' ORDER BY id DESC";
...
$table.="</tbody>";
if($valtotalamt==0){
	echo $table.",0.00,0.00";
}
else{
	echo $table.",".number_format(round($valtotalamt,2),2)."-".$valtotalQty;
