...
session_start();
	...
	FROM tw_manualdata_entry where employee_id='".$employee_id."' GROUP BY month(date), year(date) order by date";
	$retVal = $sign->FunctionJSON($qry);
	
	$qry1="select count(distinct(month(date)),(year(date))) as cnt from tw_manualdata_entry where employee_id='".$employee_id."' ";
	...
	$table="";
	$it=1;
	$table.="<thead><tr><th>SR.NO</th><th>Date</th><th>PET </th><th>Hard Plastic</th><th>Solid Paper</th><th>Hard Plastic Mixture</th><th>Soft Plastic</th><th>Reject Waste</th><th>Incoming Waste</th><th>Recycleable Waste</th></tr></thead><tbody>";
