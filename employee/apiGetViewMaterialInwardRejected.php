...
$qry="SELECT mo.id,mo.company_id,cd.CompanyName,cd.Status,cd.Company_Logo,mo.total_quantity,mo.final_total_amout,mo.reason FROM tw_material_outward mo INNER JOIN tw_company_details cd ON mo.company_id = cd.ID WHERE mo.customer_id  = '".$company_id."' and mo.po_id  = '".$po_id."' and mo.status='".$settingValueRejectedStatus."' ORDER BY mo.id DESC";
