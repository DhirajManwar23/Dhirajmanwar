...
$qry="SELECT mo.id,cd.CompanyName,mo.total_quantity,mo.final_total_amout FROM tw_material_outward mo INNER JOIN tw_company_details cd ON mo.customer_id = cd.ID WHERE mo.employee_id = '".$employee_id."'  and mo.po_id = '".$po_id."' and mo.status='".$settingValueApprovedStatus."' ORDER BY mo.id DESC";
$retVal = $sign->FunctionJSON($qry);

$qry1="Select count(*) as cnt from tw_material_outward where employee_id='".$employee_id."' and po_id = '".$po_id."' and status='".$settingValueApprovedStatus."'";
...
if($retVal1==0 || $retVal1==0.00){
	...
	$table.="<thead><tr><th>#</th><th>Customer Name</th><th>Total Quantity</th><th>Total Amount</th><th>Document</th></tr></thead><tbody>";
		while($x>=$i){
