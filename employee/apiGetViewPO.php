...
if($statustype==""){
	$qry="select id,supplier_id,status,buyer_id,po_date,delivery_date,total_quantity,final_total_amount,reason,reasontext from tw_temp_po_info where buyer_id='".$company_id."' order by id Desc";

	$qry1="select count(*) as cnt from tw_temp_po_info where buyer_id='".$company_id."'";
	
}
else{
	$qry="select id,supplier_id,status,buyer_id,po_date,delivery_date,total_quantity,final_total_amount,reason,reasontext from tw_temp_po_info where buyer_id='".$company_id."' and status='".$statustype."' order by id Desc";

	$qry1="select count(*) as cnt from tw_temp_po_info where buyer_id='".$company_id."' and status='".$statustype."'";
}
...
if($qryCnt==0 || $qryCnt==0.00){
	...
	while($x>=$i){
		...
		$qry2="Select CompanyName,Status,Company_Logo from tw_company_details where ID='".$supplier_id."'";
		...
		$qry3="Select value from tw_company_contact where company_id='".$supplier_id."'";
		...
		$qry6="Select count(*) as cnt from tw_material_outward where po_id='".$id."' and status='".$settingValuePendingStatus."'";
		...
		if($status==$settingValuePendingStatus){
			// $div = "<div class='btn-group' role='group'>
						// <a href='javascript:void(0)' onclick='AcceptRecord(".$id.")'><button type='button' class='btn btn-success'>Accept</button><a>
						// <a href='javascript:void(0)' onclick='RejectRecord(".$id.")'><button type='button' class='btn btn-danger'>Reject</button><a>
						// <a href='javascript:void(0)' onclick='ViewRecord(".$id.")'><button type='button' class='btn btn-primary'>Document</button><a>
					// </div>";
			$div = "";
			$progressdiv = "";
							 ...
							 <div class='justify-content-end d-flex'>
							  <div class='dropdown flex-md-grow-1 flex-xl-grow-0'>
								...
								<div class='dropdown-menu dropdown-menu-right' aria-labelledby='dropdownMenuDate2' style=''>
								  <a class='dropdown-item' href='#' onclick='editRecord(".$id.")'><i class='ti-pencil'></i></i> Edit</a>
		...
		$pendingOutward = "";
		if($retVal6!=0){
			$pendingOutward="(You have ".$retVal6." record in inprocess)";
		}
					...
					<div class='card-body'>
						...
						<div class='row'>
							...
							<div class='col-lg-10 col-md-10 col-sm-12 col-xs-12 col-12'>
								<strong>".$CompanyName." ".$img."</strong><a href='pgMaterialInward.php?type=In%20Process&po_id=".$id."'>".$pendingOutward."</a><p>PO No: ".$id." | Status: ".$retVal5."</p>
