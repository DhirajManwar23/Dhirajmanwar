...
$table="";
$qry=" SELECT mad.id FROM tw_epr_material_assign_details mad INNER JOIN tw_epr_material_assign_info mai ON mad.m_id=mai.id where mai.po_id='".$po_id."' AND mai.status='".$settingValuePendingStatus."' ";
$retValQry = $sign->FunctionJSON($qry);

$decodedJSON = json_decode($retValQry);
$qrycnt="SELECT COUNT(*) as cnt FROM tw_epr_material_assign_details mad INNER JOIN tw_epr_material_assign_info mai ON mad.m_id=mai.id where mai.po_id='".$po_id."' AND mai.status='".$settingValuePendingStatus."' ";
