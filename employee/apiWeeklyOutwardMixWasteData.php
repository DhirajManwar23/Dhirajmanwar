...
session_start();
   ...
   $qry="SELECT id, WEEK(entry_date) AS week,entry_date,SUM(quantity) as quantity
	FROM tw_outward_data_entry  where entry_date BETWEEN '".$StartDate."' AND  '".$EndDate."' AND ward='".$Ward."' and created_by='".$employee_id."' GROUP BY WEEK(entry_date) order by WEEK(entry_date) ";
	
	
	$qry1="select count(distinct(WEEK(entry_date))) as cnt from tw_outward_data_entry where entry_date BETWEEN '".$StartDate."' AND  '".$EndDate."' AND ward='".$Ward."' and created_by='".$employee_id."' ";
	}
$table="";
...
while($x3>=$i3){
	...
	$nameWIthComma=$FetchWasteName;
	 ...
	 $varTemp3.= "(SELECT IFNULL (sum(quantity), 00.00) FROM tw_outward_data_entry WHERE material_name='".$FetchWasteName."' AND WEEK(entry_date) = WEEK(t1.entry_date) and ward='".$Ward."' and created_by='".$employee_id."') AS '".$FetchWasteName."' ".$varComma." ";
	...
	$qryDetails=' SELECT  week(entry_date) as entry_date,'.$varTemp3.' FROM tw_outward_data_entry AS t1 where  entry_date BETWEEN "'.$StartDate.'" AND  "'.$EndDate.'" AND week(entry_date)='.$week.'
