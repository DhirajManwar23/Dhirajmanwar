...
$qrydate="SELECT id,month FROM tw_epr_approval WHERE po_id='".$po_id."' and supplier_status='".$settingValueApprovedStatus."'";
$Docqrydate = $sign->FunctionJSON($qrydate);
$decodedJSON2 = json_decode($Docqrydate);

$qrydatecnt="SELECT count(*) as cnt FROM tw_epr_approval WHERE po_id='".$po_id."' and supplier_status='".$settingValueApprovedStatus."'";
...
$qry1 = "SELECT company_status FROM tw_epr_approval WHERE po_id='".$po_id."'";
$retValCompanyStatus = $sign->selectF($qry1,"company_status");
...
foreach ($period as $dt) {
	...
	if (in_array($dt->format("M Y"),$data))
	{
		...
		//---
		if(in_array($settingValueApprovedStatus,$arrsupplier_status))
		{
			$table.="<td><img src='".$settingValueUserImagePathOther."".$settingValueic_approved."' /></td>";
		}
		else{ 
			if(in_array($settingValueRejectedStatus,$arrsupplier_status)){
		...
		//---
		if(in_array($settingValueApprovedStatus,$arrcompany_status))
		{
			$table.="<td><img src='".$settingValueUserImagePathOther."".$settingValueic_approved."' /></td>";
		}
		else{ 
			if(in_array($settingValueRejectedStatus,$arrcompany_status)){
				$table.="<td><img src='".$settingValueUserImagePathOther."".$settingValueic_rejected."' /></td>";
			}
			else{ 
				$passvalue='AcceptRecord("'.$data[1].'","'.$data[0].'")';
				$passRejvalue='RejectRecord("'.$data[1].'","'.$data[0].'")';
		...
		if($retValCompanyStatus==$settingValueApprovedStatus){
			
			$table.="<td class='text-center'><a href='pgEprTaxInvoiceDocuments.php?po_id=".$po_id."&date=".$date."' target='_blank'><i class='ti-download success'></i></a></td>";
			...
			if($AuditorCnt>0){ 
				$table.="<td class='text-center'><a href='javascript:void(0)' onclick='AuditorCertificate(".$po_id.")' title='Auditor Certificate'> <i class='ti-printer'></i></a></td>";
			}else{
				$table.="<td>-</td>";
			}
			$table.="<td class='text-center'><a href='pgViewCertificate.php' title='CPCB  Certificate' target='_blank'><i class='ti-printer'></i></a></td>";
			$table.="</tr>";
