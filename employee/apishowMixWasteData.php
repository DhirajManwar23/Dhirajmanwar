...
session_start();
	...
	FROM tw_mixwaste_manual_entry  where month(entry_date)='".$fetchmonth."' AND YEAR(entry_date) ='".$fetchyear."' GROUP BY entry_date, month(entry_date) order by entry_date ";
	$retVal = $sign->FunctionJSON($qry);
	
	$qry1="select count(distinct(entry_date),(month(entry_date))) as cnt from tw_mixwaste_manual_entry where month(entry_date)='".$fetchmonth."' AND YEAR(entry_date) ='".$fetchyear."'";
