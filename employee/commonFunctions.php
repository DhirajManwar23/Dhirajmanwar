...
class Common {
 ...
 public function getMRFSettingValue($pass){
		
	$myMRFfile = fopen("../assets/js/custom/MRF_ward_mapping.txt", "r") or die("Unable to open file!");
	//$search = "name";
	// Read from file
	$lines = file('../assets/js/custom/MRF_ward_mapping.txt');
	foreach($lines as $line)
	{
	  // Check if the line contains the string we're looking for, and print if it does
	  if(strpos($line, $pass) !== false){
...
public function getCommonDataValue($pass){
	...
	//return $this->getMRFSettingValue($pass);
 }
