...
session_start();
	...
	$queryacceptedpocount = "SELECT IFNULL (sum(replace(final_total_amout, ',', '')), 0) as acceptedpo FROM tw_material_outward where status='".$settingValueApprovedStatus."' and po_id in (select id from tw_temp_po_info where buyer_id='".$company_id."')";
	$acceptedpo = $sign->SelectF($queryacceptedpocount,"acceptedpo");
	if($acceptedpo==""){
		$acceptedpo=0.00;
	}
	
	$queryawaitingpocount = "SELECT IFNULL (sum(replace(final_total_amout, ',', '')), 0) as awaitingpocount FROM tw_material_outward where status='".$settingValuePendingStatus."' and po_id in (select id from tw_temp_po_info where buyer_id='".$company_id."')";
	$awaitingpocount = $sign->SelectF($queryawaitingpocount,"awaitingpocount");
	if($awaitingpocount==""){
		$awaitingpocount=0.00;
	}

	$queryverifiedpocount = "SELECT IFNULL (sum(total_quantity), 0) as activepo FROM tw_temp_po_info where buyer_id='".$company_id."' and status='".$settingValueApprovedStatus."' ";
	...
	$queryInprocess="select count(*) as cnt from tw_mix_waste_collection where company_id='".$company_id."' and status='".$settingValueAwaitingStatus."'  and employee_id='".$employee_id."'  order by id desc";
	$retqueryInprocess = $sign->Select($queryInprocess);
	
	$DropdownDetailsQry="SELECT DISTINCT pi.supplier_id,cd.CompanyName FROM tw_temp_po_info pi INNER JOIN tw_company_details cd ON pi.supplier_id=cd.ID where pi.buyer_id='".$company_id."' ";
        ...
        <div class="content-wrapper"> 
		 ...
		 <div class="row">
                ...
                <div class="col-lg-7 col-md-7 col-sm-7 com-xs-7 col-7">
					...
					<h6 class="font-weight-normal mb-0"><?php echo $designation_value; ?><br> <?php echo $CompanyName; ?> <?php if($retqueryInprocess>0){ ?>You have<span class="text-primary"> <a href="pgSegregationList.php" class="text-success"><br><?php echo $retqueryInprocess; ?>  segregation for approval</a><img class="New-img" src="<?php echo $settingValueEmployeeImagePathOther.$settingValueNewimage ?>"  /></span><?php } ?></h6>
                </div>
				...
				<div class="card">
					<div class="card-body">
					 <div class="d-flex justify-content-between">
					  <p class="card-title">Monthly Mix Waste Data Entry</p>
					 </div>
		 ...
		 <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                 <div class="d-flex justify-content-between">
                  <p class="card-title">Piechart</p>
                 </div>
                  <div id="epr-legend" class="chartjs-legend mt-4 mb-2"></div>
                  <canvas id="epr-pieChart" width="958" height="478" style="display: block; height: 319px; width: 639px;" class="chartjs-render-monitor"></canvas>
                </div>
              </div>
            </div>
        </div> 
...
function getMonthlyCollectionData(){
	...
	$.ajax({
			type:"POST",
			url:"apigetMonthlyBuyerReport.php",
			data:{val:val},
			success:function(response){
...
function getMixWasteMonthlyGraph(){
	$.ajax({
			...
			success:function(response){
				...
				var xValues = ["April "+yearadd,"May "+yearadd,"June "+year,"July "+year,"Aug "+year,"Sep "+year,"Oct "+year,"Nov "+year,"Dec "+year,"Jan"+yearadd,"Feb"+yearadd,"Mar"+yearadd];
