...
$qry7 = "select id,GROUP_CONCAT(CompanyName) as cpnyname from tw_company_details where ID in (SELECT DISTINCT(pi.supplier_id) from tw_po_info pi INNER JOIN tw_po_details pd ON pi.id = pd.po_id WHERE pi.company_id='".$company_id."' AND state='".$stateid."')";
$cpnyname = $sign->FunctionOption($qry7,"",'cpnyname','id');
?>
<!DOCTYPE html>
<html lang="en">
                  ...
                  <h4 class="card-title">Generate Document</h4>
						<div class="form-group row">
							<div class="col-ld-6 col-md-6 col-sm-6 col-xs-6 col-6">
								...
								<select id="txtVendor" class="form-control form-control-sm float-right" onchange="myFunction()">
							...
							<div class="col-ld-6 col-md-6 col-sm-6 col-xs-6 col-6">
								<label class="col-sm-12">Select PO</label>
								<select id="txtPO" class="form-control form-control-sm float-right" onchange="myFunctionPO()">
...
var valstateid='<?php echo $stateid;?>';
var valpo_id="";
...
function myFunction(){
	valvaendorid = $('#txtVendor').val();
	$.ajax({
		type:"POST",
		url:"apiGetPOdetails.php",
		data:{valvaendorid:valvaendorid},
		success:function(response){
			$("#txtPO").val(response);
			$("#txtPO").html("<option value=''>Select PO</option>" +response);
		}
	});  
}
