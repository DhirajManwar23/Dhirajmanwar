...
*/
    ...
    $qry="select sum(replace(plant_quantity, ',', '')) as plant_quantity,category_name,dispatched_state,material_name from tw_temp WHERE po_id='".$requestid."' AND plant_wbs_date like '".$date."%' AND status='".$settingValueCompletedStatus."' AND dispatched_state='".$StateName."'   ORDER BY id Asc";
	...
	while($x>=$i){
		...
		<?php 
		$valtotalQty = $valtotalQty+$quantity;
