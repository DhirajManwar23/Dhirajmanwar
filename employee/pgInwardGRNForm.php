...
$company_address = $decodedJSON->response[4]->company_address;
	...
	if($requesttype=="add")
	{
		$qry="select date,remark,supplier_id from tw_material_inward_grn where inward_id='".$inward_id."'";
		$retVal = $sign->FunctionJSON($qry);
		$decodedJSON2 = json_decode($retVal);
		 if(isset($decodedJSON3->response[0]->date))
		{
			$date = $decodedJSON2->response[0]->date;
		}
		else
		{
			$date="";
		}
		 if(isset($decodedJSON3->response[1]->remark))
		{
			$remark = $decodedJSON2->response[1]->remark;
		}
		else
		{
			$remark="";
		}
		if(isset($decodedJSON3->response[2]->supplier_id))
		{
			$supplier_id = $decodedJSON2->response[2]->supplier_id;
		}
		else
		{
			$supplier_id="";
	...
	$bill=$baddress_line1.",<br>".$baddress_line2.",<br>".$blocation.",".$bpincode.",<br>".$bcity.",".$bstate.",".$bcountry;
	...
	$ship=$saddress_line1.",<br>".$saddress_line2.",<br>".$slocation.",".$spincode.",<br>".$scity.",".$sstate.",".$scountry;
				  ...
				  <div class="form-group">
						<div class="row">
							<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 col-12">
								...
								<input type="text" disabled class="form-control" id="txtinvoicenumber" 
					...
					<div class="form-group">
					   ...
					   <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 col-12">
							...
							<input type="text" disabled class="form-control" id="txtsuppliername"  maxlength="30" value=" <?php echo $CustomerCompanyName; ?>" />
                       </div>
					   <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12 col-12">
					...
					<div class="form-group">
                      ...
                      <textarea type="text" class="form-control" id="txtremark" maxlength="5000" placeholder="Remark"> 
					...
					<br>
					<button type="button" class="btn btn-success" id="btnAddrecord" <?php echo $disabledGRN;?> onclick="adddata();"><?php if($requesttype=="add"){ ?>Add<?php }else{ ?>Update<?php } ?></button>					
 ...
 function adddata(){
	   if(!validateBlank($("#txtDate").val())){
		...
		if(valrequesttype=="add"){
			...
			valquery = "insert into tw_material_inward_grn(company_id,supplier_id,supplier_address,company_address,date,created_by,created_on,created_ip) values('"+companyID+"','"+$("#txtsuppliername").val()+"','"+$("#txtsupplieraddress").val()+"','"+$("#txtcompanyaddress").val()+"','"+$("#txtDate").val()+"','"+valcreated_by+"','"+valcreated_on+"','"+valcreated_ip+"')";
			
			valquerycount = "select count(*) as cnt from tw_material_inward_grn where supplier_address='"+$("#txtsupplieraddress").val()+"'";
		
		}
		
		else{
			 var valrequestid = "<?php echo $requestid;?>";
			valquery = "update tw_material_inward_grn set supplier_address='"+$("#txtsupplieraddress").val()+"',company_address='"+$("#txtcompanyaddress").val()+"',supplier_id='"+$("#txtsuppliername").val()+"',date='"+$("#txtDate").val()+"',modified_by='"+valcreated_by+"', modified_on='"+valcreated_on+"', modified_ip='"+valcreated_ip+"' where id='"+valrequestid+"'";
			valquerycount = "select count(*) as cnt from tw_material_inward_grn where supplier_address='"+$("#txtsupplieraddress").val()+"' and ID!='"+valrequestid+"'";
		} 
	...
	$.ajax({
			...
			success:function(response){
				...
				if($.trim(response)=="Success"){
					if(valrequesttype=="add"){
						showAlertRedirect("Success","Record Added Successfully","success","pgInwardGRN.php?inward_id="+valinward_id);
					}
					else{
						showAlertRedirect("Success","Record Updated Successfully","success","pgInwardGRN.php?inward_id="+valinward_id);
					}
				}
		...
		disableButton('#btnAddrecord','<i class="ti-timer"></i> Processing...');
		var valquery = "insert into tw_material_inward_grn(inward_id,company_id,supplier_id,date,remark,created_by,created_on,created_ip) values('"+varinward_id+"','"+valcompany_id+"','"+valcustomer_id+"','"+$("#txtDate").val()+"','"+$("#txtremark").val()+"','"+valcreated_by+"','"+valcreated_on+"','"+valcreated_ip+"')";
		
		$.ajax({
			type:"POST",
			url:"apiAddMaterialInwardGRN.php",
			data:{inward_id:varinward_id,company_id:valcompany_id,txtsuppliername:valcustomer_id,txtDate:$("#txtDate").val(),txtremark:$("#txtremark").val()},
			success:function(response){
				console.log(response);
				if(valrequesttype=="add"){
					enableButton('#btnAddrecord','Add Record');
