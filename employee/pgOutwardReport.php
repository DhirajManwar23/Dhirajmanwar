		...
		<div class="row" id="Chart">
				...
				<div class="card">
					<div class="card-body">
					 <div class="d-flex justify-content-between">
					  <p class="card-title">Outward dry waste collection Data in Kgs</p>
					 </div>
			...
			<div class="col-lg-6 col-md-6 col-sm-12 col-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                 <div class="d-flex justify-content-between">
                  <p class="card-title">Outward dry waste Segregation data in Kgs</p>
                 </div>
          ...
          <div class="row" id="Report">
            <div class="col-md-12 grid-margin stretch-card">
              <div class="card">
                <div class="card-body">
                  <h4 class="card-title">Outward Dry Waste Collection Record<span id="showType" ></span> <button type="button" class="btn btn-link btn-rounded btn-fw float-right" onclick="ShowReport();"><i class="ti-import"></i></button></h4>
...
function buildReports() {
	...
	$.post("api-outward-report-graph.php", request, function(response) {
...
function buildDonutChart(dounutChartData) {
	...
	dounutChartData.forEach((item) => {
		...
		wasteTypesArray.push(item.name);
...
function showMonthlyData(StartDate,EndDate,Ward){
	$.ajax({
		type:"POST",
		url:"apiMonthlyOutwardWasteData.php",
...
function showWeeklyData(StartDate,EndDate,Ward){
	$.ajax({
		type:"POST",
		url:"apiWeeklyOutwardMixWasteData.php",
		data:{year:year,StartDate:StartDate,EndDate:EndDate,Ward:Ward},
		success:function(response){
			console.log(response);
			$("#tableData").html(response);
...
function ViewMonthlyRecord(id,date,Ward){
	window.open("pgDailyOutwardReport.php?id="+id+"&date="+date+"&Ward="+Ward, '_blank');
} 
...
function ShowReport(){
	...
	if(ReportType=="Weekly"){
		...
		window.open('pgDownloadWeeklyOutwardReport.php?year=&StartDate='+StartDate+'&EndDate='+EndDate+"&Ward="+Ward, '_blank');
	}
	else if(ReportType=="Daily"){
			window.open('pgDownloadDailyOutwardReport.php?year=&StartDate='+StartDate+'&EndDate='+EndDate+"&Ward="+Ward, '_blank');
	}
	else if(ReportType=="Yearly"){
		window.open('pgDownloadYearlyOutwardReport.php?year=&StartDate='+StartDate+'&EndDate='+EndDate+"&Ward="+Ward, '_blank');
	}
	else if(ReportType=="Monthly"){
		window.open('pgDownloadMonthlyOutwardReport.php?year=&StartDate='+StartDate+'&EndDate='+EndDate+"&Ward="+Ward, '_blank');
	}
}
//Daily Start
function showData(StartDate,EndDate,Ward){
	$.ajax({
		type:"POST",
		url:"apiGetViewOutwardDataEntry.php",
...
function showDataEntry(entry_date){
    $.ajax({
		type:"POST",
		url:"apishowoutwardMixWasteData.php",
...
function editRecord(id,date1){
	window.location.href = "pgOutwardForm.php?type=edit&id="+id+"&date="+date1;
...
function deleteYes(id)
{
		$.ajax({
			type:"POST",
			url:"apiDeleteOutwardManualData.php",
			data:{id:id},
			success:function(response){
				console.log(response);
				if($.trim(response)=="Success"){
					//showData();
					showAlertRedirect("Success","Record Deleted Successfully","success","pgOutwardReport.php");
				}
				else{
