<?php 
	...
	//--Karuna Invoice Amount Start
		...
		if($retValInvoice>0){
			$qryInvoice="SELECT id,document,document_value,amount from tw_material_outward_documents WHERE type='Invoice' and outward_id='".$requestid."' ORDER BY outward_id ASC";
			...
			$qry1Invoice="SELECT id,invoice_number,final_total_amount from tw_tax_invoice WHERE outward_id='".$requestid."' ORDER BY outward_id ASC";
		...
		$valInfo = $retValmo." | ".$invoice_number." | &#8377;".$invoiceamount." | Balance Amount: &#8377;".number_format(round($Balanceamount,2),2); 
		//---Karuna Invoice Amount End
?>
<!DOCTYPE html>
<html lang="en">
              ...
              <div class="card">
                ...
                <div class="card-body">
                  ...
                  <h4 class="card-title"><?php echo $valInfo; ?></h4>
					<!---<button type="button" class="btn btn-link btn-rounded btn-fw float-right" onclick="location.href='pgPaymentListForm.php?type=add&id=&outward_id=<?php echo $requestid;?>';"><i class="icon-plus"></i> Create New Record</button>-->
...
var valoutwardid="<?php echo $requestid; ?>";
	...
	function showData(){
		$.ajax({
			type:"POST",
			url:"apiGetViewPaymentListSupplier.php",
