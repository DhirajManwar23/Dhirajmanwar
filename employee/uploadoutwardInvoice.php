...
//upload.php
 ...
 if (file_exists($_FILES['Document_ProofInvoice']["tmp_name"]))
{
 //$test = explode('.', $_FILES["Document_ProofInvoice"]["name"]);
 $name = ($_FILES["Document_ProofInvoice"]["name"]);
 //$ext = end($test);
// $name = date("dmYhis"). '.' . $ext;
 $location = '../assets/images/Documents/Employee/Outward/' . $name;  
 move_uploaded_file($_FILES["Document_ProofInvoice"]["tmp_name"], $location);
 echo $name; 
