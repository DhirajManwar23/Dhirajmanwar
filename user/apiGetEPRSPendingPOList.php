...
if($type=="Pending"){
	...
	$qry="SELECT DISTINCT(pi.id),pi.po_number,pi.company_id,pi.status,pi.date_of_po,pi.total_quantity,pi.final_total_amount,pi.reason FROM tw_po_info pi INNER JOIN tw_temp tt ON tt.po_id=pi.id WHERE pi.supplier_id='".$company_id."' and tt.status='".$settingValueRejectedStatus."' order by pi.id Desc";
	$qry1="select count(DISTINCT(pi.id)) as cnt FROM tw_po_info pi INNER JOIN tw_temp tt ON tt.po_id=pi.id WHERE pi.supplier_id='".$company_id."' and tt.status='".$settingValueRejectedStatus."'";
}
...
if($qryCnt==0 || $qryCnt==0.00){
	...
	while($x>=$i){
		...
		if($type=="Approved"){
			...
			$ViewPO = "<a href='pgViewPODetails.php?po_id=".$id."' title='More Info' class='float-right'><i class='ti-arrow-right'></i></a>";
			...
			".$ViewFullfilmentPendingRecords." ".$ViewFullfilmentRecords." ".$ViewFullfilmentAcceptRecords." ".$ViewPO." 
			"; */
			$ViewAuditor = "<a href='pgAssignAuditor.php?id=".$id."' onclick='AssignAuditorPO(".$id.")' title='View' class='float-right'><i class='ti-user'></i> Assign Auditor</a>";
			$divatag = $ViewPO.$ViewAuditor;
			...
			if($qryCntRejected!=0){
				$ViewFullfilmentRejectedRecords="<a href='javascript:void(0)' onclick='ViewFullfilmentRejectedRecords(".$id.")' title='View Queries' class='float-right'></i><i class='ti-arrow-right'></i></a>";
			}
			else{
			...
			$qryCompleted="select count(*) as cnt from tw_temp where po_id='".$id."' and status = '".$settingValueCompletedStatus."'";
			$qryCompleted = $sign->Select($qryCompleted);
			if($qryCompleted!=0){
				$ViewFullfilmentRejectedRecords="<a href='javascript:void(0)' onclick='AuditorCertificate(".$id.")' title='View Fulfilled'><i class='ti-eye'></i> View Auditor Certificate</a>";
			}
			else{
				$ViewFullfilmentRejectedRecords="";
			}
					...
					<div class='card-body'>
						<div class='row'>
							<div class='col-lg-2 col-md-2 col-sm-12 col-xs-12 col-12' style='text-align:center;'>
								<img src='".$Company_Logo."' width='100%' style='width:100px;height:100px;' class='img-sm mb-3 '><br><a href='javascript:void(0)' onclick='ViewRecord(".$id.")' title='View PO'><i class='ti-printer'></i> View PO</a>
							</div>
							<div class='col-lg-10 col-md-10 col-sm-12 col-xs-12 col-12'>
								<strong>".$CompanyName.$img."".$complianceimg." [".$retqrystate."]"."</strong>
								".$divatag."
								<p>PO No : ".$po_number." | Status: ".$retVal5."</p>
								 <p><i class='ti-calendar'></i> PO Date: ".date("d-m-Y", strtotime($date_of_po))." | <i class='ti-calendar'></i> Delivery Date: ".date("d-m-Y", strtotime($delivery_date))." | Pending Days: Difference is: ".$difference/(24*60*60)." days</p>
