...
if($ID=="0"){
	...
	//----
	$qry = "select DISTINCT (category_name) from tw_temp where po_id in (select id from tw_po_info where supplier_id='".$company_id."')";
	$retVal = $sign->FunctionJSON($qry);
	$qrycnt = "select COUNT(DISTINCT(category_name)) as cnt from tw_temp where po_id in (select id from tw_po_info where supplier_id='".$company_id."')";
	...
	while($x>=$i){
		...
		$qry1="SELECT IFNULL(SUM(replace(tt.quantity, ',', '')),0) as mcount FROM tw_po_details tt INNER JOIN tw_po_info tpi ON tt.po_id=tpi.id WHERE tt.product_id in (SELECT id from tw_epr_product_master WHERE epr_category_id='".$category_id."') and tpi.supplier_id='".$company_id."'";
	...
	$queryverifiedpocount = "SELECT IFNULL (sum(total_quantity), 0) as activepo FROM tw_po_info where company_id='".$ID."' and status='".$settingValueApprovedStatus."'  and supplier_id='".$company_id."'";
	$activepo = $sign->SelectF($queryverifiedpocount,"activepo");
	if($activepo==""){
		$activepo=0.00;
	}
	$queryawaitingpocount = "SELECT IFNULL (sum(replace(plant_quantity, ',', '')), 0) as awaitingpocount FROM tw_temp where status='".$settingValueAwaitingStatus."'  and po_id in (select id from tw_po_info where company_id='".$ID."' and supplier_id='".$company_id."')";
	$awaitingpocount = $sign->SelectF($queryawaitingpocount,"awaitingpocount");
	if($awaitingpocount==""){
		$awaitingpocount=0.00;
	}
	
	$queryacceptedpocount = "SELECT IFNULL (sum(replace(plant_quantity, ',', '')), 0) as acceptedpo FROM tw_temp where status='".$settingValueCompletedStatus."' and po_id in (select id from tw_po_info where company_id='".$ID."' and supplier_id='".$company_id."')";	
	$acceptedpo = $sign->SelectF($queryacceptedpocount,"acceptedpo");
	if($acceptedpo==""){
		$acceptedpo=0.00;
	}
	...
	$queryrejectedbycompanycount = "SELECT COUNT(*) as rejectedbycompany FROM tw_temp where status='".$settingValueRejectedStatus."' and po_id in (select id from tw_po_info where company_id='".$ID."' and supplier_id='".$company_id."') and rejected_by='Company'";
	$rejectedbycompany = $sign->SelectF($queryrejectedbycompanycount,"rejectedbycompany");
	if($rejectedbycompany==""){
		$rejectedbycompany=0.00;
	}
	
	$queryrejectedbyauditorcount = "SELECT COUNT(*) as rejectedbyauditor FROM tw_temp where status='".$settingValueRejectedStatus."' and po_id in (select id from tw_po_info where company_id='".$ID."' and supplier_id='".$company_id."') and rejected_by='Auditor'";
	$rejectedbyauditor = $sign->SelectF($queryrejectedbyauditorcount,"rejectedbyauditor");
	if($rejectedbyauditor==""){
		$rejectedbyauditor=0.00;
	}
	//----
	$qry = "select DISTINCT (category_name) from tw_temp where po_id in (select id from tw_po_info where supplier_id='".$company_id."' and company_id='".$ID."')";
	$retVal = $sign->FunctionJSON($qry);
	$qrycnt = "select COUNT(DISTINCT(category_name)) as cnt from tw_temp where po_id in (select id from tw_po_info where supplier_id='".$company_id."' and company_id='".$ID."')";
	$retValcnt = $sign->select($qrycnt);
	...
	while($x>=$i){
		...
		$qry1="SELECT IFNULL(SUM(replace(tt.quantity, ',', '')),0) as mcount FROM tw_po_details tt INNER JOIN tw_po_info tpi ON tt.po_id=tpi.id WHERE tt.product_id in (SELECT id from tw_epr_product_master WHERE epr_category_id='".$category_id."') and tpi.supplier_id='".$company_id."' and tpi.company_id='".$ID."'";
