...
$qry="select id,receiver_id,sender_id,connect_date,connect_status from tw_company_network where receiver_id='".$company_id."' AND connect_status='".$PendingStatus."' order by id Desc";
$retVal = $sign->FunctionJSON($qry);

$qry1="Select count(*) as cnt from tw_company_network where receiver_id='".$company_id."' AND connect_status='".$PendingStatus."'";
...
while($x>=$i){
    ...
    $query="SELECT cd.ID,cd.CompanyName,cd.Company_Logo,cc.value,cd.CompanyType FROM tw_company_details cd INNER JOIN tw_company_contact cc ON cc.company_id = cd.id WHERE cd.id ='".$sender_id."' and cc.contact_field='".$PendingStatus."'";
	...
	if ($companyStatus==$verifyStatus) { 
		...
		$img="";
			...
			<button type='button' class='btn btn-primary' onclick='viewCompanyProfile(".$sender_id.")'><i class='ti-info'></i></button>
			<button type='button' class='btn btn-success' onclick='accept(".$sender_id.");'><i class='ti-check btn-icon-prepend'></i> </button>
