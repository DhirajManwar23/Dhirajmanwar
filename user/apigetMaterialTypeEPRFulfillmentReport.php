...
FROM tw_temp
where po_id in (select id from tw_po_info where supplier_id='".$company_id."' ".$varCondition.") and status='".$settingValueCompletedStatus."' and purchase_invoice_date>='2022-04-01' and purchase_invoice_date<='2023-03-31'
GROUP BY category_name";
	$retValState = $sign->FunctionJSON($qryState);
	$qryStatecount = "SELECT COUNT(distinct(category_name)) as ds FROM tw_temp
	where po_id in (select id from tw_po_info where supplier_id='".$company_id."' ".$varCondition.") and status='".$settingValueCompletedStatus."' and purchase_invoice_date>='2022-04-01' and purchase_invoice_date<='2023-03-31'";
