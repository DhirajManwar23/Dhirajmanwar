...
if($requesttype=="add"){
	...
	$qry1 = "SELECT id,document_type_value from tw_document_type_master where visibility='true' and id not in (select DISTINCT(document_type) from tw_company_document where company_id='".$created_by."') order by priority asc";
	$retVal1 = $sign->FunctionOption($qry1,$Status,'document_type_value','id');
	
}
else{
	$qry3 = "SELECT cd.document_type,cd.document_number, cd.document_proof, cd.document_verification_status,cd.document_size,cd.documennt_purpose, cd.created_by, cd.created_on, cd.created_ip, cd.modified_by, cd.modified_on, cd.modified_ip   FROM tw_company_document cd LEFT JOIN  tw_company_details cc ON cd.company_id = cc.id WHERE cd.id ='".$requestid."' ";
