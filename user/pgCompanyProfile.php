...
$settingValuePotherEmail=$sign->escapeString($settingValuePotherEmail);
...
$settingUserBankImage  =$commonfunction->getSettingValue("BankImage");
...
$queryEPRServices = "SELECT CompanyType FROM tw_company_details where ID='".$company_id."'";
$EPRServices = $sign->SelectF($queryEPRServices,"CompanyType");
...
<body>
  <div class="container-scroller">
    ...
    <div class="container-fluid page-body-wrapper">
	 ...
	 <div class="modal fade" data-keyboard="false" data-backdrop="static" id="modalOtp" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	  ...
	  <!-- ==============MODAL END ================= -->
	   <!-- ==============MODAL START ================= -->
  <div class="modal fade" data-keyboard="false" data-backdrop="static" id="modalEditLogo" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
		<div class="modal-header">
  ...
  <!-- ==============COMPANY NAME MODAL END ================= -->
	 ...
	 <div class="main-panel">        
        <div class="content-wrapper">
          <div class="row">
            <div class="col-lg-4 col-md-4 grid-margin">
              <div class="card">
                <div class="card-body">
					<div style="text-align:center;">
						...
						<a id="OpenImgUpload"><img src="<?php if($Company_Logo==""){echo $settingValueUserImagePathOther.$settingValueUserCompanyImage; }else{ echo $settingValueUserImagePathVerification.$userid."/".$Company_Logo;}?>" class="img-lg rounded-circle mb-3" />  </a>
						<br>
						<?php if($Company_Logo==""){ 
						 } else{  ?>
						<a class="text-info pointer-cursor" onclick="deletecompanyImge(<?php echo $created_by; ?>,'<?php echo $Company_Logo  ?>');"" >   <i class="ti-trash"></i>   </a>
						 <?php  }   ?>
						<br>
						...
						<?php if($compliance_status==$settingValueVerifiedStatus){?><img src="<?php echo $CommonDataValueCommonImagePath.$ComplianceImage;?>" data-toggle="tooltip" data-placement="top" title="This is a compliant company"/> <?php }?>
						</h1>
					...
					<div class="row">
						<div class="col-lg-10 col-md-10 col-sm-10 col-xs-10 col-10">	
							...
							<p class="card-description" id="Uservalue"><?php echo $pvalue; ?><br>
								...
								<small class="text-success"><img src="<?php echo $settingValueUserImagePathOther.$VerifiedGreenTickImage ?>" /> Verified</small>
								<?php  }?>
			  ...
			  <br>
			  <?php if($EPRServices!=$settingValueEPRServices)
			  { ?>
			  <div class="card">
                <div class="card-body">
					<h4 class="card-title">Team Members</h4>
						<?php
								$qry2 = "SELECT cc.employee_name, cc.employee_photo ,cfm.department_name,td.designation_value,cc.id  FROM tw_employee_registration cc LEFT JOIN tw_department_master cfm ON cc.employee_department = cfm.id LEFT JOIN tw_designation_master td
								...
								while($x>=$i){
										...
										$count=$count+1;
										$emp_id  = $decodedJSON2->response[$count]->id ;
										$count=$count+1;
										
										$qry4="Select value from tw_employee_contact where employee_id = '".$emp_id."' and contact_field='".$settingValuePemail."'";
										$retVal4 = $sign->SelectF($qry4,"value");
											
							?>
					<div class="row">
						<div class="col-lg-3 col-md-3 col-sm-3 col-xs-3 col-3">
							<img src="<?php if($employee_photo==""){echo $settingValueEmployeeImagePathOther.$settingValueEmployeeImage; }else{ echo $settingValueEmployeeImagePathVerification.$retVal4."/".$employee_photo;}?>" class="img-sm" loading="lazy" />
						</div>
            ...
            <div class="col-lg-8 col-md-8 grid-margin">
				<div class="card">
					<div class="card-body">
						<h4 class="card-title"><i class="ti-info-alt"></i> Description<a href="pgCompanyDescription.php" class="float-right">
			  ...
			  <div class="card">
                <div class="card-body">
					...
					<hr>
					<?php
						$qry2 = "SELECT cc.id,cc.value,cc.status,ctm.contact_type,ctm.contact_icon,cc.contact_field   FROM tw_company_contact cc LEFT JOIN tw_contact_field_master ctm ON cc.contact_field = ctm.id WHERE company_id = '".$company_id."' and ( contact_field!='".$settingValuePemail."' and contact_field!='".$settingValuePcontact."' )";
						$retVal2 = $sign->FunctionJSON($qry2);
						$decodedJSON2 = json_decode($retVal2);
			  ...
			  <div class="card">
                <div class="card-body">
					...
					<hr>
					<?php
						$qry2 = "SELECT cb.id,cb.bank_name,cb.branch_location,cb.bank_account_status,ba.bank_account_type_value,cb.bank_account_proof FROM tw_company_bankdetails cb LEFT JOIN tw_bank_account_type_master ba ON cb.account_type  = ba.ID WHERE company_id = '".$company_id."'";
						$retVal2 = $sign->FunctionJSON($qry2);
						$decodedJSON2 = json_decode($retVal2);
					  ...
					  <div class="row">
						<div class="col-lg-2 col-md-2 col-sm-12 col-xs-12 col-12">
							...
							<img src="<?php echo $settingValueUserImagePathOther.$settingUserBankImage;?>" class="img-sm" loading="lazy" />
						</div>
						<div class="col-lg-9 col-md-9 col-sm-11 col-xs-11 col-11">
						 ...
						 <?php }
							...
							<?php }
								...
								<?php $reasonBankQry="SELECT rm.reason FROM `tw_company_bankdetails` cd INNER JOIN tw_rejected_reason_master rm ON cd.remark=rm.id where cd.company_id='".$company_id."'  AND cd.bank_account_status='".$settingValueRejectedStatus."' AND cd.id='".$id."'";
								$Bankreason=$sign->SelectF($reasonBankQry,'reason');
						
							?>
							<small class="text-danger"><i class="ti-na"></i> Rejected (<em><?php echo $Bankreason;?></em>)</small>
			  ...
			  <?php $docCntQry="SELECT count(document_type) as cnt from tw_company_document where company_id='".$company_id."'";
                    $docCnt=$sign->Select($docCntQry);			 
                     
					$DocMasterCntQry="SELECT count(*) as cnt FROM `tw_document_type_master` where visibility='true'";
					$DocMasterCnt=$sign->Select($DocMasterCntQry);

                    if($docCnt==$DocMasterCnt){
						
					
					?>
			 <div class="card">
                <div class="card-body">
					<h4 class="card-title"> <i class="ti-file"></i> KYC Documents<a  class="float-right"></a></h4>
					<hr>
					<?php 
					}
					else{					
			  ...
			  <div class="card">
                <div class="card-body">
					<h4 class="card-title"> <i class="ti-file"></i> KYC Documents<a href="pgAddCompanyDocumentsDetails.php?type=add&id=" class="float-right"><i class="ti-plus"></i></a></h4>
					<hr>
					  <?php
					}
						$qry2 = "SELECT ctm.id,ctm.document_number, ctm.document_verification_status, dtm.document_type_value,ctm.document_proof FROM tw_company_document ctm LEFT JOIN tw_document_type_master dtm ON ctm.document_type = dtm.ID WHERE company_id = '".$company_id."'";
						$retVal2 = $sign->FunctionJSON($qry2);
						$decodedJSON2 = json_decode($retVal2);
					  ...
					  <div class="row">
						<div class="col-lg-11 col-md-11 col-sm-11 col-xs-11 col-11">
							<strong><?php echo $document_type_value;?></strong> <a href="<?php echo $settingValueUserImagePathVerification; ?><?php echo $EMAIL."/".$document_proof;?>" target="_blank"><i class="ti-file"></i><a/><br>
						 ...
						 <?php }
							...
							<?php $reasonQry="SELECT rm.reason FROM `tw_company_document` cd INNER JOIN tw_rejected_reason_master rm ON cd.remark=rm.id where cd.company_id='".$company_id."'  AND cd.document_verification_status='".$settingValueRejectedStatus."' AND cd.id='".$id."'";
							$reason=$sign->SelectF($reasonQry,'reason');
						
							?>
							
							<small class="text-danger"><i class="ti-na"></i> Rejected (<em><?php echo $reason;?></em>)</small>
							<?php
							}
							else { ?></p>
							<small class="text-warning"><i class="ti-alert"></i> </small>
		  ...
		  <br>
		  <?php $docCntQry="SELECT count(document_type) as cnt from tw_compliance_document where company_id='".$company_id."'";
                    $docCnt=$sign->Select($docCntQry);			 
                     
					$DocMasterCntQry="SELECT count(*) as cnt FROM tw_compliance_type_master where visibility='true'";
					$DocMasterCnt=$sign->Select($DocMasterCntQry);
		...
		<div class="card">
        <div class="card-body">
					<h4 class="card-title"> <i class="ti-file"></i> Compliance Documents<a  class="float-right"></a></h4>
					<hr>
					<?php 
					}
					else{					
			...
			<div class="card">
                <div class="card-body">
					<h4 class="card-title"> <i class="ti-file"></i> Compliance Documents<a href="pgAddComplianceDocumentType.php?type=add&id=" class="float-right"><i class="ti-plus"></i></a></h4>
					<hr>
					  <?php
					}
						$qry2 = "SELECT cd.id, cd.document_number, cd.document_verification_status, ctm.compliance_document_type, cd.document_verification_status, cd.document_proof FROM tw_compliance_document cd, tw_compliance_type_master ctm where ctm.id = cd.document_type and cd.company_id = '".$company_id."'";
						$retVal2 = $sign->FunctionJSON($qry2);
						$decodedJSON2 = json_decode($retVal2);
					  ...
					  <div class="row">
						<div class="col-lg-11 col-md-11 col-sm-11 col-xs-11 col-11">
						 ...
						 <?php }
							...
							<?php $reasonQry="SELECT rm.reason FROM  tw_compliance_document cd INNER JOIN tw_rejected_reason_master rm ON cd.remark=rm.id where cd.company_id='".$company_id."'  AND cd.document_verification_status='".$settingValueRejectedStatus."' AND cd.id='".$id."'";
							$reason=$sign->SelectF($reasonQry,'reason');
						...
						<div class="col-lg-1 col-md-1 col-sm-1 col-xs-1 col-1">
							<a href="pgAddComplianceDocumentType.php?type=edit&id=<?php echo $id;?>" class="float-right"><i class="ti-pencil"></i></a>
						</div>
 ...
 $.ajax({
		...
		data:{value:email, id:id, contact:contact,otp:result,User_type:User_type},
		  success:function(response){
			console.log(response);
		if($.trim(response)=="Success"){
			showAlertRedirect("Success","Otp Verified Successfully","success","pgCompanyProfile.php");
		}
		else if($.trim(response)=="error"){
			showAlert("Warning","Invalid Otp..","warning");
			$("#first").focus();
			$("#first").val("");
			$("#second").val("");
			$("#third").val("");
...
function deletecompanyImge(id,name){
	...
	var Imagename="";
						...
						showConfirmAlert('Confirm action!', 'Are you sure you want to delete the image?','question', function (confirmed){
							if(confirmed==true){
							showAlertRedirect("Success","Company Logo Deleted Successfully","success","pgCompanyProfile.php");
...
var valcreated_ip = "<?php echo $ip_address;?>";
		...
		success:function(response){
			...
			if($.trim(response)=="Success"){
				showAlertRedirect("Success","Company Logo Updated Successfully","success","pgCompanyProfile.php");
			}else{
