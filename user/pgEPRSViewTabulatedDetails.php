...
<table class="printtbl">
  ...
  <th>Quantity (KG) </th>
  <?php 
	$detailsQry="select aggeragator_name,dispatched_state,IFNULL (sum(replace(plant_quantity, ',', '')), 0) as plant_quantity FROM tw_temp where po_id='".$po_id."' and dispatched_state='".$statename."' and status='".$settingValueCompletedStatus."' and plant_wbs_date like '".$rdatepo_id."%' GROUP by aggeragator_name";
