...
$Status = "";
$qry = "select id,company_type from tw_company_type_master where visibility = 'true' order by priority,id";
$retVal = $sign->FunctionOption($qry,$Status,'company_type','id');

$email = "";
$u1 = $_REQUEST["u1"];
$token = $_REQUEST["v2"];

$qry1 = "select count(*) as cnt from tw_company_contact where value = '".$u1."'";
...
<title>Trace-Waste | User Create an Account</title>
<!-- plugins:css -->
            ...
            <div class="auth-form-light text-left py-5 px-4 px-sm-5">
              ...
              <div class="pt-3">
                <div class="form-group">
                  <input type="text" class="form-control form-control-lg" maxlength="50" id="txtCompanyName" placeholder="Company Name">
                </div>
				<div class="form-group">
					<select id="selCompany_Type" class="form-control form-control-sm">
						<?php echo $retVal; ?>
					</select>
				</div>
                <div class="form-group">
                  <input type="email" class="form-control form-control-lg" id="txtEmail" value="<?php echo $email; ?>" placeholder="Email">
                </div>
                <div class="form-group">
                ...
                <div class="text-center mt-4 font-weight-light">
                  Already have an account? <a href="pgLogin.php" class="text-primary">Login</a>
                </div>
              </div>
            </div>
          </div>
...
function register(){
	if(!validateBlank($("#txtCompanyName").val())){
		setError("txtCompanyName");
	}
	else if(!validateBlank($("#selCompany_Type").val())){
		setErrorOnBlur("selCompany_Type");
	} 
		...
		$.ajax({
			...
			data:{companyname:$("#txtCompanyName").val(),
				companytype:$("#selCompany_Type").val(),
				email:$("#txtEmail").val(),
				...
				if($.trim(response)=="Success"){
					...
					$("#txtConfirmPassword").val("");
					$("#selCompany_Type").val("");
					showAlertRedirect("Success","Register Details saved Successfully","success","pgLogin.php");
				}  
