...
//upload.php
 ...
 if (file_exists($_FILES['Document_Proof_Approved_By']["tmp_name"]))
{
 
	 $name = ($_FILES["Document_Proof_Approved_By"]["name"]);

	 $location = $UserImagePathVerification.$retVal.'/'. $name;  
	 move_uploaded_file($_FILES["Document_Proof_Approved_By"]["tmp_name"], $location);
	 echo $name; 
