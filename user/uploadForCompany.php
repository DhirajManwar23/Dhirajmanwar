...
//upload.php
 ...
 if (file_exists($_FILES['Document_Proof_For_Company']["tmp_name"]))
{
 
 $name = ($_FILES["Document_Proof_For_Company"]["name"]);

 $location = $UserImagePathVerification.$retVal.'/'. $name;  
 move_uploaded_file($_FILES["Document_Proof_For_Company"]["tmp_name"], $location);
 echo $name; 
